...
function readInput(day::Integer)
    path = joinpath(@__DIR__, "..", "data", @sprintf("input%02d.txt", day))
    return readInput(path)
end
export readInput
function readExample(day::Integer)
    path = joinpath(@__DIR__, "..", "data", @sprintf("example%02d.txt", day))
    return readInput(path)
end
export readExample
...
for d in solvedDays
    global ds = @sprintf("input%02d.txt", d)
...
function benchmark(days=solvedDays)
    ...
    for day in days
        ...
        input = readInput(joinpath(@__DIR__, "..", "data", @sprintf("input%02d.txt", day)))
        @eval begin
