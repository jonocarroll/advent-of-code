...
f25a <- function(x) {
  g <- gdf(x)
  seps <- igraph::min_cut(g, value.only = FALSE)
  length(seps$partition1) * length(seps$partition2)
}

gdf <- function(x) {
  x <- do.call(rbind.data.frame, strsplit(x, ": "))
  colnames(x) <- c("prim", "sec")
  tidyr::separate_longer_delim(x, sec, " ") |> 
