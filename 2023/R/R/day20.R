...
f20a <- function(x) {
  fromto <- strsplit(x, " -> ")
  invisible(lapply(fromto, \(z) makefun(x, z)))
  rx <<- function(p) NULL
  output <<- function(p) NULL
  low <<- 0
  high <<- 0
  for (i in 1:1000) {
    queue <<- vector(mode = "list", length = 500)
    qi <<- 1
    queue[[qi]] <<- list("broadcaster" = FALSE, from = "broadcaster")
    while(qi > 0) {
     nextval <- queue[[1]]
     calledby <<- unlist(nextval[2])[[1]]
     nextval <- unlist(nextval[1])
     queue[[1]] <<- NULL
     qi <<- qi - 1
     if (nextval) high <<- high + 1 else low <<- low + 1
     do.call(names(nextval), list(unname(nextval)))
    }
  }
  sprintf("%.15g", low * high)
}

makefun <- function(x, s) {
  type <- sapply(s[1], \(w) switch(substr(w, 1, 1), 
                                    "&" = "conj",
                                    "%" = "flip",
                                    "b" = "broadcast"))
  src <- sub("[&%]", "", s[1])
  ...
  if (type == "conj") {
    ...
    #' that input. Then, if it remembers *high pulses* for all inputs, it sends
    #' a *low pulse*; otherwise, it sends a *high pulse*
    destvals <- paste0(dest, "_val")
    for (d in destvals) {
      assign(d, FALSE, envir = .GlobalEnv)
    }
    inputvals <- get_inputs(x, src)
    assign(paste0(src, "_lastseen"), as.list(setNames(rep(FALSE, length(inputvals)), inputvals)), envir = .GlobalEnv)
    eval(str2lang(glue::glue("|src| <- function(p) {
               |src|_lastseen[[calledby]] <<- p
               outval <- !do.call(all, |src|_lastseen)
               |paste('qi <<- qi + 1; queue[[qi]] <<- base::c(',dest, ' = outval, from = match.call()[[1]])', collapse = '\\n')|
               outval
               }", .open = "|", .close = "|")), envir = .GlobalEnv)
  } else if (type == "flip") {
    #' *Flip-flop* modules (prefix `%`) are either *on* or *off*; they are
    #' initially *off*. If a flip-flop module receives a high pulse, it is
    #' ignored and nothing happens. However, if a flip-flop module receives a
    ...
    eval(str2lang(glue::glue("|src| = function(p) {
               if (p) { return(NULL) }
               outval <- !get('|src|_val', envir = .GlobalEnv)
               assign('|src|_val', outval, envir = .GlobalEnv)
               |paste(' qi <<- qi + 1; queue[[qi]] <<- base::c(', dest, ' = outval, from = match.call()[[1]])', collapse = '\\n')|
               outval
               }", .open = "|", .close = "|")), envir = .GlobalEnv)
    ...
    eval(str2lang(glue::glue("|src| <- function(p) {
               |paste(' qi <<- qi + 1; queue[[qi]] <<- base::c(', dest, ' = p, from = match.call()[[1]])', collapse = '\\n')|
               p
               }", .open = "|", .close = "|")), envir = .GlobalEnv)
  }
}

get_inputs <- function(x, s) {
  substring(sub(" ->.*", "", grep(paste0("->.*?", s), x, value = TRUE)), 2)
}
...
f20b <- function(x) {
  # get_inputs(x, "rx") = "gq"
  # gq sends low when all inputs to that send high
  # get_inputs(x, "gq") = "xj", "qs", "kz", "km"
  fromto <- strsplit(x, " -> ")
  invisible(lapply(fromto, \(z) makefun(x, z)))
  rx <<- function(p) NULL
  output <<- function(p) NULL
  ...
  for (i in 1:10000) {
    ...
    while(qi > 0) {
      ...
      queue[[1]] <<- NULL
      qi <<- qi - 1
      res <- do.call(names(nextval), list(unname(nextval)))
      if (names(nextval) %in% vvals && res) {
        message("found ", names(nextval), " at ", i)
        vv[[names(nextval)]] <- i
      }
    }
    if (all(unlist(vv) > 0)) {
  ...
  if (!done) stop("not enough")
  sprintf("%.16g", Reduce(pracma::Lcm, vv))
}
