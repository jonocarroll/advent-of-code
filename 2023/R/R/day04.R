...
f04b <- function(x) {
  ...
  cards <- rep(1, l)
  for (i in 1:l) {
    wins <- f04_helper(x[i], points = FALSE)
    if (wins == 0) next
    for (j in min(l,i+1):min(l,i+wins)) {
...
f04_helper <- function(x, points = TRUE) {
  card <- as.integer(sub("Card[ ]*?(\\d+).*", "\\1", x))
  xx <- sub("Card.*?: ", "", x)
  nums <- as.integer(strsplit(trimws(sub("\\|.*$", "", xx)), "[ ]+")[[1]])
  winning <- as.integer(trimws(strsplit(sub("^.*?\\| ", "", xx), "[ ]+")[[1]]))
  count <- length(intersect(winning, nums))
  if (!points) return(count)
  if (count == 0) {
    0
  } else if (count == 1) {
    1
