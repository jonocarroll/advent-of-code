...
f09_helper <- function(x) {
  d <- 1
  vals <- diffs <- list()
  vals[[d]] <- as.integer(strsplit(x, " ")[[1]])
  while (!all(vals[[d]] == 0)) {
    vals[[d+1]] <- diffs[[d+1]] <- na.omit(vals[[d]] - dplyr::lag(vals[[d]]))
    d <- d + 1
  }
  vals[[d+1]] <- 0
  for (i in rev(seq_along(vals))[-1]) {
    lastval <- vals[[i]][length(vals[[i]])]
    vals[[i]] <- c(vals[[i]], lastval + vals[[i+1]][length(vals[[i+1]])])
  }
  vals[[1]][length(vals[[1]])]
}

f09_helper_2 <- function(x) {
  d <- 1
  vals <- diffs <- list()
  vals[[d]] <- as.integer(strsplit(x, " ")[[1]])
  while (!all(vals[[d]] == 0)) {
  ...
  for (i in rev(seq_along(vals))[-1]) {
    firstval <- vals[[i]][1]
    vals[[i]] <- c(firstval - vals[[i+1]][1], vals[[i]])
  }
  vals[[1]][1]
}
