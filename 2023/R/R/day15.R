...
f15b <- function(x) {
  ...
  ids <- sub("[-=].*$", "", x)
  ops <- sub("^.*?([-=].*$)", "\\1", x)
  boxes <- vector(mode = "list", length = length(ids))
  for (i in seq_along(ids)) {
    hash <- f15_helper(ids[i])
    if (ops[i] == "-") {
      if (hasName(boxes[[hash+1]], ids[i])) {
        boxes[[hash+1]] <- boxes[[hash+1]][names(boxes[[hash+1]]) != ids[i]]
      }
      next
    }
    focal <- substr(ops[i],2,2)
    if (ids[i] %in% names(boxes[[hash+1]])) {
      boxes[[hash+1]] <- replace(boxes[[hash+1]], names(boxes[[hash+1]]) == ids[i], focal)
    } else {
      boxes[[hash+1]] <- append(boxes[[hash+1]], setNames(focal, ids[i]))
    }
  }
  ans <- 0
  for (b in seq_along(boxes)) {
    if (length(boxes[[b]]) == 0) next
    ans <- ans + sum(b*sapply(seq_along(boxes[[b]]), \(z) z*as.integer(boxes[[b]][z])))
  }
  ans
}
...
f15_helper <- function(x) {
  ...
  ans <- 0
  for (char in x) {
    ans <- ((ans + as.integer(charToRaw(char)))*17) %% 256
  }
  ans
}
