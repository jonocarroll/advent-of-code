...
decimal <- function(x) {
  digits <- strsplit(x, "")[[1]]
  translate <- c("=" = -2, "-" = -1, "0" = 0, "1" = 1, "2" = 2)
  decimal <- translate[match(digits, names(translate))]
  sum(rev(decimal) * 5^(0:(length(decimal)-1)))
}

snafu <- function(x) {
  digits <- rev(as.integer(convert_base(x)))

  while (any(digits > 2)) {
    for (i in seq_along(digits)) {
      if (digits[i] > 2) {
        digits[i] <- digits[i] - 5
        if (i + 1 > length(digits)) {
          digits[i+1] <- 0
  ...
  translate <- c("=" = -2, "-" = -1, "0" = 0, "1" = 1, "2" = 2)
  # print(rev(digits))
  paste0(names(translate)[match(rev(digits), translate)], collapse = "")

}

# # based on https://stackoverflow.com/a/70910640/4168169
# loses precision
# convert_base <- function(x, base = 5) {
#   ifelse(x < base, x, Recall(x %/% base) * 10 + x %% base)
# }

# https://stackoverflow.com/a/64380219/4168169
convert_base <- function(x, base = 5) {
...
example_data_25 <- function(example = 1) {
  l <- list(
    a = readLines("inst/example25.txt")
  )
  l[[example]]
}
