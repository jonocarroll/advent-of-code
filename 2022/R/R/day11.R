...
#' Away](https://en.wikipedia.org/wiki/Keep_away){target="_blank"} with
#' your missing things!
#'
...
#' business]{title="Monkey business monkey business monkey business, monkey numbers... is this working?"}
...
#' f11b()
f11a <- function(x, parta = TRUE) {
  monkeys <- strsplit(paste(x, collapse = "\n"), "\n\n")[[1]]
  monkeys <- lapply(monkeys, extract_monkey)
  inspected <- vector(mode = "list", length = length(monkeys))
  inspected[] <- 0
  maxdiv <- prod(unlist(lapply(monkeys, \(y) y[["test"]])))
  n <- ifelse(parta, 20, 10000)
  for (round in 1:n) {
    for (m in seq_along(monkeys)) {
      thism <- monkeys[[m]]
      if (length(thism$items) > 0) {
        for (i in thism$items) {
          inspected[[m]] <- inspected[[m]] + 1
          if (parta) {
            worry <- trunc(thism$op(i)/3)
          } else {
            worry <- trunc(thism$op(i))
          }
          # to <- ifelse(modulo2(worry, thism$test), thism$istrue, thism$isfalse) + 1
          to <- ifelse(worry %% thism$test == 0, thism$istrue, thism$isfalse) + 1
          monkeys[[to]]$items <- c(monkeys[[to]]$items, worry %% maxdiv)
          monkeys[[m]]$items <- monkeys[[m]]$items[-1]
        }
      }
    }
  }
  most <- tail(sort(unlist(inspected)), 2)
  prod(most)
  # inspected
}

modulo2 <- function(x, y) {
  while (x >= 0) {
    x <- x - y
    if (x == 0) return(TRUE)
  }
...
extract_monkey <- function(x) {
  fields <- trimws(strsplit(x, "\\n")[[1]])
  num <- as.integer(sub("Monkey ([0-9]*):", "\\1", fields[1]))
  start <- eval(parse(text = paste0("c(", sub("Starting items: ", "", fields[2]), ")")))
  op <- eval(parse(text = paste0("function(old) {", sub("Operation: ", "", fields[3]), "; new}")))
  test <- as.integer(sub("Test: divisible by ", "", fields[4]))
  istrue <- as.integer(sub("If true: throw to monkey ", "", fields[5]))
  isfalse <- as.integer(sub("If false: throw to monkey ", "", fields[6]))
  list(
    monkey = num,
    items = start,
    op = op,
