...
#' *[quantum]{title="Please disregard the wave interference patterns that would arise from the wave-particle duality of individual tachyon particles while repairing the manifold."}
#' tachyon manifold*.
#'
...
#'     ......|........
#'
#' Or, there\'s the timeline where the particle ends up at the same point
#' as the alternating timeline, but takes a totally different path to get
#' there:
#'
#'     .......S.......
#'     .......|.......
...
f07a <- function(x) {
  ...
  # d <- readLines("inst/input07.txt")
  d <- x
  grid <- matrix(unlist(strsplit(d, "")), ncol = nchar(d[1]), byrow = TRUE)
  beams <- which(grid == "S", arr.ind = TRUE)
  r <- beams[1]
  acc <- 0
  while (r < nrow(grid)) {
    newbeams <- matrix(NA, nrow = 0, ncol = 2)
    for (b in seq_len(nrow(beams))) {
      this_beam <- as.matrix(t(beams[b,]), ncol = 2)
      if (grid[this_beam + c(1, 0)] == "^") {
        newbeams <- rbind(newbeams, this_beam + c(1, -1), this_beam + c(1, 1))
        acc <- acc + 1
      } else {
        newbeams <- rbind(newbeams, this_beam + c(1, 0))
      }
    }
    r <- r + 1
    beams <- unique(newbeams)
  }
  acc
}
...
f07b <- function(x) {
  ...
  while (r < nrow(grid)) {
    ...
    for (b in seq_len(nrow(beams))) {
      this_beam <- as.matrix(t(beams[b,]), ncol = 2)
      if (grid[this_beam] == "^") {
        hm[this_beam[2] - 1] <- hm[this_beam[2] - 1] + hm[this_beam[2]]
        hm[this_beam[2] + 1] <- hm[this_beam[2] + 1] + hm[this_beam[2]]
        hm[this_beam[2]] <- 0
        newbeams <- rbind(newbeams, this_beam + c(0, -1), this_beam + c(0, 1))
      } else {
        newbeams <- rbind(newbeams, this_beam)
      }
    }
    r <- r + 1
    beams <- t(t(unique(newbeams)) + c(1, 0))
  }
  sprintf("%16.0f", sum(hm))
}
