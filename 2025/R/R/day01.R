...
#'
#' The good news is that they\'ve discovered [project
#' management](https://en.wikipedia.org/wiki/Project_management){target="_blank"}!
#' This has given them the tools they need to prevent their usual Christmas
#' emergency. For example, they now know that the North Pole decorations
#' need to be finished soon so that other critical tasks can start on time.
#'
#' The bad news is that they\'ve realized they have a *different*
#' emergency: according to their resource planning, none of them have any
#' time left to decorate the North Pole!
#'
#' To save Christmas, the Elves need *you* to *finish decorating the North
#' Pole by December 12th*.
#'
...
#' decorating. Unfortunately, the *password* seems to have been changed, so
#' you can\'t get in. A document taped to the wall helpfully explains:
#'
...
#' numbers `0` through `99` in order. As you turn the dial, it makes a
...
#' the dial to point at `19`. After that, a rotation of `L19` would cause
#' it to point at `0`.
#'
#' Because the dial is a circle, turning the dial *left from `0`* one click
#' makes it point at `99`. Similarly, turning the dial *right from `99`*
#' one click makes it point at `0`.
#'
...
#' North Pole secret entrance security training seminar taught you that the
#' safe is actually a decoy. The actual password is *the number of times
...
#'
#' \"Due to newer security protocols, please use *password method
#' [0x434C49434B]{title="You should have seen the chaos when the Elves overflowed their 32-bit password method counter."}*
...
#' causes the dial to point at `0`, regardless of whether it happens during
#' a rotation or at the end of one.
#'
...
#' back to `50`!
#'
#' Using password method 0x434C49434B, *what is the password to open the
#' door?*
#'
#' @param x some data
