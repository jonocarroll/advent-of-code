...
#'
#' So, by
#' [carefully](https://en.wikipedia.org/wiki/Racing_the_Beam){target="_blank"}
#' [[timing]{title="While you're at it, go watch everything else by Retro Game Mechanics Explained, too."}](https://www.youtube.com/watch?v=sJFnWZH5FXc){target="_blank"}
...
f10a <- function(x) {
  all_ops <- fill_df(x)
  all_ops$signal <- all_ops$cycle * all_ops$lastval
  interesting <- seq(20, max(all_ops$cycle), 40)
  sum(all_ops[all_ops$cycle %in% interesting, "signal"])
}
...
f10b <- function(x) {
  crt <- fill_df(x)
  crt <- crt[crt$cycle > 0, ]
  crt$pixels <- ifelse(abs(((crt$cycle - 1) %% 40) - (crt$lastval)) < 2, "#", ".")
  screen <- split(crt$pixels, gl(nrow(crt)/40, 40))
  screen <- lapply(screen, paste0, collapse = "")
  for (r in screen) {
    cat(r, "\n")
  }
}
...
fill_df <- function(x) {
  ops <- lapply(x, f10_helper)
  start <- data.frame(op = "start", add_cycle = 0, add_val = 1)
  all_ops <- rbind(start, do.call(rbind, ops))
  all_ops$cycle <- cumsum(all_ops$add_cycle)
  all_ops$val <- cumsum(all_ops$add_val)
  # lag by 1 op
  all_ops$lastval <- c(1, all_ops$val[-nrow(all_ops)])
  all_ops
}

f10_helper <- function(x) {
  if (x == "noop") return(data.frame(op = "noop", add_cycle = 1, add_val = 0))
  val <- sub("addx ", "", x)
  return(data.frame(op = c(x, "wait"), add_cycle = c(1, 1), add_val = c(0, as.integer(val))))
}
