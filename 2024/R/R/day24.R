...
f24a <- function(x) {
  ...
  blank <- which(x == "")
  starts <- x[1:(blank-1)]
  others <- x[(blank+1):length(x)]
  starts <- strsplit(starts, ": ")
  inputs <- lapply(starts, \(z) as.integer(z[[2]]))
  names(inputs) <- sapply(starts, \(z) z[[1]])
  
  others <- strsplit(others, " -> ")
  otherinputs <- lapply(others, \(z) z[[2]])
  otherinputs[] <- NA
  names(otherinputs) <- lapply(others, \(z) z[[2]])
  
  gates <- lapply(others, \(z) z[[1]])
  gates <- lapply(gates, \(z) strsplit(z, " ")[[1]])
  gates1 <- lapply(gates, \(z) z[[1]])
  gates1[] <- NA
  names(gates1) <- lapply(gates, \(z) z[[1]])
  gates2 <- lapply(gates, \(z) z[[3]])
  gates2[] <- NA
  names(gates2) <- lapply(gates, \(z) z[[3]])

  wires <- c(inputs, otherinputs, gates1, gates2)
  zwires <- wires[startsWith(names(wires), "z")]
  
  while (any(is.na(unlist(wires[startsWith(names(wires), "z")])))) {
    for (o in others) {
      inwire <- strsplit(o[1], " ")[[1]]
      outwire <- o[2]
      w1 <- wires[[inwire[1]]]
      w2 <- wires[[inwire[3]]]
      ...
      wires[[outwire]] <- as.integer(op(as.integer(w1), as.integer(w2)))
    }
  }
  
  zz <- unlist(wires[startsWith(names(wires), "z")])
  rev(zz[sort(names(zz))])
  print(bits_to_num(zz[sort(names(zz))]), digits = 22)
}
...
f24b <- function(x) {
  ...
  gate_df <- purrr::map_df(others, \(z) as.data.frame(t(strsplit(z, " ")[[1]]))) |> 
    dplyr::mutate(V4 = sapply(others, \(z) z[2]))
  names(gate_df) <- c("in1", "op", "in2", "out")

  # If the output of a gate is z, then the operation has to be XOR 
  # unless it is the last bit.  
  z_not_XOR <- dplyr::filter(gate_df, 
                             out != "z45" & 
                               startsWith(out, "z") & 
                               op != "XOR")
  ...
  not_z_not_XOR <- dplyr::filter(gate_df, 
                                 op == "XOR" & 
                                   !startsWith(out, "z") & 
                                   (!startsWith(in1, "x") & !startsWith(in1, "y"))
  ) 
  
  # If you have a XOR gate with inputs x, y, there must be another XOR gate 
  # with this gate as an input. 
  XOR_x_y <- dplyr::filter(gate_df, op == "XOR" & 
                             (startsWith(in1, "x") | startsWith(in1, "y")))
  XOR_not_XOR_input <- c()
  for (z in XOR_x_y$out) {
    tmp <- dplyr::filter(gate_df, op == "XOR" & (in1 == z | in2 == z))
    if(nrow(tmp) == 0) {
      XOR_not_XOR_input <- c(XOR_not_XOR_input, z)
    }
  }
  
  AND_not_lsb <- dplyr::filter(gate_df, 
                                    op == "AND" & 
                                      in1 != "x00" & 
                                      in1 != "y00" & 
                                      in2 != "x00" & 
                                      in2 != "y00")
  AND_not_OR_input <- c()
  for (z in AND_not_lsb$out) {
    tmp <- dplyr::filter(gate_df, op == "OR" & (in1 == z | in2 == z))
    if(nrow(tmp) == 0) {
      AND_not_OR_input <- c(AND_not_OR_input, z)
    }
  }
