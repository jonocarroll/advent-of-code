...
f10a <- function(x) {
  ...
  mode(x) <- "integer"
  p <- matrix(1:length(x), nrow = nrow(x), byrow = T)
  starts <- which(x==0, arr.ind = T)
  ends <- which(x==9, arr.ind = T)
  adj <- can_reach(x)
  library(igraph)
  g <- graph_from_adjacency_matrix(adj)
  tot <- 0
  for (th in 1:nrow(starts)) {
    s <- starts[th, ]
    asp <- all_shortest_paths(g, from = p[matrix(s, nrow = 1)], to = p[ends])
    n <- length(unique(sapply(asp$vpaths, \(y) tail(y, 1))))
    tot <- tot + n
  }
  tot
}
...
can_reach <- function(ngrid) {
  ...
  for (v in 1:(nrows * ncols)) {
    ...
    for (w in 1:(nrows * ncols)) {
      ...
      if (row_diff <= 1 && col_diff <= 1 && row_diff + col_diff == 1) {
        this <-  ngrid[y[1], y[2]] - ngrid[i, j] == 1
        if (!is.na(this) && this == 1) {
