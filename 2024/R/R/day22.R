...
#' f22b()
f22a <- function(x) {
  x <- as.integer(readLines("inst/input22.txt"))

  library(future)
  library(furrr)
  plan(multisession)
  # do it faster
  sims <- furrr::future_map(x, gen_nums, .progress = TRUE)
  ...
  for (i in seq_along(prices)) {
    ...
    for (j in seq_along(prices[[i]])) {
      ...
      if (is.null(gethash(seen_i, s))) {
        if (is.null(val <- gethash(hm, s))) {
          sethash(hm, s, this_p)
        } else (
          sethash(hm, s, c(val, this_p))
        )
        sethash(seen_i, s, 1)
      }
    }
...
gen_nums <- function(s, n = 2000) {
  ...
  evolve <- memoise::memoise(function(s) {
    s = bitwXor((s * 64) %% 16777216, s %% 16777216)
    s = bitwXor(trunc(s / 32) %% 16777216, s %% 16777216)
    bitwXor((s * 2048)  %% 16777216, s %% 16777216)
  })
