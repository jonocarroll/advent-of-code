...
f05a <- function(x) {
  ...
  dsp <- which(d=="")
  drules <- strsplit(d[1:(dsp-1)], "\\|")
  dupdates <- strsplit(d[(dsp+1):length(d)], ",")
  
  rules <- vector(mode = "list", length = 100)
  for (r in seq_along(drules)) {
    rules[[as.integer(drules[[r]][1])]] <- c(rules[[as.integer(drules[[r]][1])]], as.integer(drules[[r]][2]))
  }
  
  is_correct <- function(x) {
    res <- TRUE
    for (i in head(seq_along(x), -1)) {
      after <- as.integer(x[(i+1):length(x)])
      res <- res && all(after %in% rules[[as.integer(x[i])]])
    }
    res
  }
  
  tot <- 0
  for (good in which(sapply(dupdates, is_correct))) {
    x <- dupdates[[good]]
    mid <- (length(x)+1)/2
    tot <- tot + as.integer(x[mid])
  }
...
f05b <- function(x) {
  ...
  repair <- function(x) {
    ...
    for (i in head(seq_along(x), -1)) {
      after <- as.integer(x[(i+1):length(x)])
      anyv <- setdiff(after, rules[[as.integer(x[i])]])
      if (length(anyv)) {
        viol <- c(viol, anyv)
      }
    }
  ...
  notgood <- which(!sapply(dupdates, is_correct))
  tot <- 0
  for (ng in notgood) {
    x <- dupdates[[ng]]
    xx <- x
